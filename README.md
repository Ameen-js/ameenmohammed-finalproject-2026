...
## Executive Summary
This project is an automated, serverless data ingestion pipeline built on AWS. It lets you upload CSV files that contain student grades, which are processed and kept in a database.

**Problem Solved:** Educational institutions often rely on manual data entry for student records. This project allows for the automated processing and upload of student grade information, while keeping the costs very low if not $0, by utlizing AWS Free Tier
...
1. Upload a test CSV file structured with headers: `student_id, course, grade, term`.
2. The S3 upload automatically triggers the Lambda function.
3. Verify the records are populated in the DynamoDB table.
4. Check CloudWatch logs for successful execution or error handling metrics.
